...
import { X, Send, ChevronRight, Globe } from 'lucide-react';
...
export default function AiSupportWidget() {
  ...
  // Widget খুললে প্রথমবার data fetch হবে — page load-এ নয়
  useEffect(() => {
  ...
  // Toggle with smooth closing animation
  const handleToggle = () => {
    if (isOpen && !isClosing) {
      setIsClosing(true);
      setTimeout(() => {
  ...
  const handleToggleLanguage = () => {
    ...
    setMessages(prev => [...prev, {
      ...
      sender: 'bot',
      text: nextLang === 'bn'
        ? `🌐 ভাষা পরিবর্তন করা হয়েছে: **বাংলা**। যেকোনো প্রশ্ন বা শর্টকার্ট টাইপ করুন।`
  ...
  const getAiResponse = (userQuery, currentLang) => {
    ...
    const getDynamicMembershipsText = () => {
      ...
      if (membershipList.length > 0) {
        let text = isBn
          ? `💼 **বিক্রয়নাউ প্রিমিয়াম মেম্বারশিপ প্ল্যানসমূহ (লাইভ আপডেট):**\n\n`
    ...
    const getDynamicPaymentText = () => {
      ...
      return isBn
        ? `💳 **বিকাশ/নগদ পেমেন্ট করার নিয়ম:**\n\n১. [মেম্বারশিপ পেজে](/membership) গিয়ে আপনার পছন্দের বুস্ট বা মেম্বারশিপে ক্লিক করুন।\n২. আমাদের অফিশিয়াল নম্বরে${numbersInfo} পেমেন্ট (Send Money) সম্পন্ন করুন।\n৩. আপনার বিকাশ/নগদ নম্বর ও ট্রানজেকশন আইডি (TxnID) দিয়ে ফরমটি সাবমিট করুন।\n৪. অ্যাডমিন পেমেন্ট ভেরিফাই করে ১ মিনিটে প্যাকেজ একটিভ করে দেবেন।`
    ...
    if (hasAny(['boost', 'bst', 'top', 'promot', 'promote', 'fast sale', 'fast sell', 'বুস্ট', 'টপ', 'সেল', 'তাড়াতাড়ি', 'বিক্রি', 'কুইক', 'সুপার'])) {
    ...
    if (hasAny(['post', 'pst', 'add', 'ad', 'dibo', 'দিবো', 'পোস্ট', 'ফ্রি', 'free', 'বিজ্ঞাপন', 'পোষ্ট', 'পস্ট', 'লাগবে', 'নতুন অ্যাড', 'ক্রিয়েট'])) {
      return isBn
        ? `📢 **বিজ্ঞাপন পোস্ট করার নিয়ম:**\n\nবিক্রয়নাউতে প্রতি মাসে আপনি **৩টি বিজ্ঞাপন সম্পূর্ণ ফ্রিতে** পোস্ট করতে পারবেন!\n\n১. ওপরের **'+ Post Ad'** বাটনে ক্লিক করুন।\n২. আপনার পণ্যের ক্যাটাগরি, ছবি, সঠিক দাম ও জেলা সিলেক্ট করুন।\n৩. আপনার ফোন নম্বর ও বর্ণনা দিয়ে সাবমিট করুন।\n\n👉 [বিজ্ঞাপন পোস্ট করতে এখানে ক্লিক করুন](/post-ad)`
    ...
    if (hasAny(['call', 'chat', 'cntct', 'contact', 'num', 'nongbor', 'number', 'phone', 'mobile', 'seller', 'কল', 'চ্যাট', 'নাম্বার', 'নম্বর', 'মোবাইল', 'ফোন', 'যোগাযোগ', 'বিক্রেতা'])) {
      return isBn
        ? `💬 **বিক্রেতার সাথে যোগাযোগ করার নিয়ম:**\n\n১. যেকোনো বিজ্ঞাপনের ওপর ক্লিক করে ডিটেইলস পেজে যান।\n২. নিচে **'Chat with Seller'** বাটনে ক্লিক করে সরাসরি চ্যাট করতে পারবেন অথবা **'Call Seller'** বাটনে চাপ দিয়ে মোবাইল নম্বর পাবেন।`
    ...
    if (hasAny(['edit', 'edt', 'del', 'delete', 'change', 'myad', 'manage', 'এডিট', 'ডিলেট', 'মুছব', 'আমার অ্যাড', 'পরিবর্তন'])) {
      return isBn
        ? `📝 **বিজ্ঞাপন পরিবর্তন/মুছে ফেলার নিয়ম:**\n\nআপনার পোস্ট করা সকল বিজ্ঞাপন দেখতে এবং এডিট করতে আপনার প্রোফাইলের **My Ads (আমার বিজ্ঞাপন)** সেকশনে যান। সেখান থেকে এক ক্লিকেই অ্যাড এডিট বা ডিলেট করতে পারবেন।\n\n👉 [আপনার বিজ্ঞাপনে যান](/my-ads)`
        : `📝 **Manage Your Ads:**\n\nTo edit, update, or delete your posted ads, go to your **My Ads** dashboard.\n\n👉 [Go to My Ads](/my-ads)`;
    }

    if (hasAny(['login', 'lgn', 'signup', 'reg', 'register', 'otp', 'pass', 'password', 'acc', 'account', 'লগইন', 'সাইনআপ', 'রেজিস্টার', 'ওটিপি', 'পাসওয়ার্ড', 'একাউন্ট'])) {
      return isBn
        ? `🔐 **একাউন্ট তৈরি ও লগইন করা:**\n\n১. ইমেইল ও নাম দিয়ে সাইনআপ করলে আপনার ইমেইলে ৬ ডিজিটের ওটিপি (OTP) কোড পাঠানো হবে।\n২. ওটিপি কোডটি বসালেই অটোমেটিক রেজিস্টার সম্পন্ন হয়ে যাবে।\n৩. এছাড়া আপনি ১-ক্লিকে **Continue with Google** দিয়েও সরাসরি লগইন করতে পারবেন।\n\n👉 [লগইন পেজে যান](/login) | [সাইনআপ পেজে যান](/signup)`
    ...
    if (hasAny(['loc', 'location', 'dist', 'district', 'dhaka', 'search', 'find', 'জেলা', 'লোকেশন', 'সার্চ', 'খুঁজব', 'ঢাকা', 'চট্টগ্রাম'])) {
      return isBn
        ? `📍 **জেলা ও লোকেশন অনুযায়ী পণ্য খোঁজা:**\n\nওয়েবসাইটের ওপরে বাম কোণায় **'সমগ্র বাংলাদেশ'** বাটনে ক্লিক করে দেশের যেকোনো ৬৪টি জেলার যেকোনো একটি সিলেক্ট করে সেই এলাকার বিজ্ঞাপনগুলো দেখতে পারবেন।`
        : `📍 **Filter by 64 Districts:**\n\nClick **'All of Bangladesh'** at the top left to filter listings by any specific district!`;
    }

    if (hasAny(['pend', 'pending', 'time', 'active', 'delay', 'late', 'এক্টিভ', 'পেন্ডিং', 'সময়', 'দেট', 'অনুমোদন'])) {
      return isBn
        ? `⏱️ **পেমেন্ট ভেরিফিকেশন ও এক্টিভেশন সময়:**\n\nবিকাশ/নগদ পেমেন্ট সাবমিট করার পর আমাদের অ্যাডমিন প্যানেল **১ থেকে ৫ মিনিটের মধ্যে** পেমেন্ট চেক করে আপনার মেম্বারশিপ বা বুস্ট একটিভ করে দেবে।`
    ...
    if (hasAny(['safe', 'safety', 'scam', 'fraud', 'cheat', 'সেফ', 'নিরাপত্তা', 'প্রতারণা', 'দালাল'])) {
      return isBn
        ? `🛡️ **নিরাপদে কেনাবেচার জরুরি টিপস:**\n\n১. পণ্য না দেখে বা ডেলিভারি নেওয়ার আগে কাউকে অগ্রিম টাকা দেবেন না।\n২. কেনাবেচার জন্য জনবহুল ও নিরাপদ স্থান বেছে নিন।\n৩. পণ্য হাতে পেয়ে ভালোভাবে পরীক্ষা করে তারপর মূল্য পরিশোধ করুন।\n\n👉 [নিরাপত্তা গাইড বিস্তারিত পড়ুন](/stay-safe)`
    ...
    if (hasAny(['admin', 'human', 'manush', 'help', 'hlp', 'agent', 'support', 'অ্যাডমিন', 'মানুষ', 'হেল্প', 'সাপোর্ট'])) {
      return isBn
        ? `👤 **লাইভ অ্যাডমিন সাপোর্ট:**\n\nআমাদের অ্যাডমিন সাপোর্ট টিম লাইনে রয়েছে। আপনার নির্দিষ্ট প্রশ্নটি নিচে টাইপ করে রাখুন, অ্যাডমিন প্রতিচ্ছবি পাওয়ার সাথে সাথে আপনাকে রিপ্লাই দেবেন।`
  ...
  const quickPrompts = aiLang === 'bn' ? [
    ...
    { label: '💳 বিকাশ/নগদে পেমেন্ট নিয়ম', query: 'পেমেন্ট নিয়ম' },
  ] : [
  ...
  // Do not render public AI Support Widget inside Admin Dashboard or chat pages (checked AFTER all hooks)
  if (pathname?.startsWith('/admin-dashboard') || pathname?.startsWith('/chat/') || pathname === '/chat') {
    return null;
  }
  ...
  return (
    <div className={styles.floatingContainer}>
      ...
      {/* Chat Window — slides up when open, slides down when closing */}
      {(isOpen || isClosing) && (
            ...
            <div className={styles.headerLeft}>
              ...
              <div className={styles.headerTitleBox}>
                <h4 className={styles.headerTitle}>
                  {aiLang === 'bn' ? 'বিক্রয়নাউ AI অ্যাসিস্ট্যান্ট' : 'BikroyNow AI Assistant'}
                </h4>
      ...
      {/* Trigger Button — সবসময় দৃশ্যমান, click করলে toggle হয় */}
      <button
        className={`${styles.triggerBtn} ${isOpen ? styles.triggerBtnActive : ''}`}
        onClick={handleToggle}
        aria-label="AI Assistant"
      >
        {/* Animated icon: X when open, logo when closed */}
        ...
        <div className={styles.triggerText}>
          ...
          {!isOpen && <div className={styles.greenDotSmall} />}
        </div>
      </button>
