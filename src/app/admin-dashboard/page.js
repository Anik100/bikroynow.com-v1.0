...
import AdminAiWidget from '../../components/AdminAiWidget';
...
export default function AdminDashboard() {
  ...
  const [bulkFeatureLoading, setBulkFeatureLoading] = useState(false);
  ...
  const [supportEmail, setSupportEmail] = useState('support@bikroynow.com');
  const [supportWhatsapp, setSupportWhatsapp] = useState('8801700000000');
  const [supportWhatsappText, setSupportWhatsappText] = useState('Hello, I need help with BikroyNow!');
  ...
  const handleBulkFeatureByPackage = async (packageName) => {
    if (!packageName) return;
    setBulkFeatureLoading(true);
    setErrorMessage('');
    try {
      // 1. Fetch active profiles with this membership
      const { data: rawProfiles, error: profileErr } = await supabase
        .from('profiles')
        .select('id, email, membership_expires_at')
        .ilike('membership_type', `%${packageName}%`);
      ...
      const profiles = (rawProfiles || []).filter(p => {
        ...
        return new Date(p.membership_expires_at) > now;
      });

      if (profiles.length === 0) {
        setErrorMessage(lang === 'bn' ? `কোনো সক্রিয় '${packageName}' মেম্বার পাওয়া যায়নি!` : `No active '${packageName}' members found.`);
        setBulkFeatureLoading(false);
        return;
      }

      const userIds = profiles.map(p => p.id);

      // 2. Fetch all active listings for these users
      ...
      if (!activeListings || activeListings.length === 0) {
        setErrorMessage(lang === 'bn' ? `এই মেম্বারদের কোনো সক্রিয় বিজ্ঞাপন পাওয়া যায়নি!` : `No active listings found for these members.`);
        setBulkFeatureLoading(false);
        return;
      }

      // 3. Prepare bulk insert/upsert for featured_ads
      const nextSort = featuredList.length > 0 ? Math.max(...featuredList.map(item => item.sort_order || 0)) + 1 : 0;
      
      const featuredInserts = activeListings.map((lst, idx) => ({
        listing_id: lst.id,
        is_active: true,
        sort_order: nextSort + idx
      }));

      // Upsert into featured_ads
      ...
      setErrorMessage(
        lang === 'bn' 
          ? `✓ সফলভাবে ${activeListings.length}টি বিজ্ঞাপন বাল্ক ফিচার করা হয়েছে!` 
          : `✓ Successfully bulk featured ${activeListings.length} listings!`
      );
      
      fetchFeaturedList();
    } catch (err) {
      console.error('Error in bulk feature:', err);
      setErrorMessage('Exception: ' + err.message);
    } finally {
      setBulkFeatureLoading(false);
    }
  ...
  const handleSetPromotionType = async (adId, promoType) => {
    setActionLoading(adId + '-promo');
    try {
      const currentTime = new Date().toISOString();
      const isVerifiedVal = promoType ? true : false;
      const updates = {
      ...
      if (!error) {
        ...
        setErrorMessage(
          lang === 'bn' 
            ? `✓ বিজ্ঞাপনের প্রমোশন স্ট্যাটাস সফলভাবে আপডেট করা হয়েছে! (${promoType || 'সাধারণ'})` 
            : `✓ Promotion status updated to ${promoType || 'Regular'}!`
        );
      } else {
        setErrorMessage('Error updating promotion: ' + error.message);
      }
    } catch (err) {
      setErrorMessage('Exception: ' + err.message);
    } finally {
      ...
      <div className="container">
        ...
        {errorMessage && (
          <div style={{
            ...
            width: 'calc(100% - 2rem)',
            maxWidth: '520px',
            background: errorMessage.startsWith('✓') || errorMessage.startsWith('⚙️') || errorMessage.includes('সফলভাবে') ? '#059669' : '#dc2626',
            color: 'white',
            ...
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>{errorMessage}</span>
            <button 
              onClick={() => setErrorMessage(null)} 
              style={{ background: 'rgba(255,255,255,0.25)', border: 'none', color: 'white', cursor: 'pointer', fontWeight: 'bold', width: '26px', height: '26px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: '0.5rem', flexShrink: 0 }}
            >
              ✕
            </button>
          </div>
        ...
        <div className={styles.header}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
            ...
            <button 
              ...
              title={lang === 'bn' ? 'হোম পেজে ফিরে যান' : 'Return to Home Page'}
            >
              <LogOut size={16} /> {lang === 'bn' ? 'হোম পেজে যান' : 'Go to Home'}
            </button>
          </div>
        </div>
        ...
        <div ref={adminContentRef} style={{ scrollMarginTop: '80px' }} />
        ...
        {activeTab === 'featured_ads' && (
          <div style={{
            ...
            <div style={{
              ...
              <h4 style={{ margin: '0 0 0.75rem 0', color: '#334155', fontSize: '0.85rem', fontWeight: 800 }}>
                {lang === 'bn' ? '👥 মেম্বারশিপ প্যাকেজ অনুযায়ী বাল্ক ফিচার করুন' : '👥 Bulk Promote Listings by Membership'}
              </h4>
              <p style={{ color: '#64748b', fontSize: '0.78rem', margin: '0 0 1rem 0', fontWeight: 500, lineHeight: 1.4 }}>
                {lang === 'bn' 
                  ? 'নির্দিষ্ট মেম্বারশিপ প্যাকেজের (Gold, Silver, Business) সকল মেম্বারদের সকল সক্রিয় বিজ্ঞাপনকে এক ক্লিকে বিজ্ঞাপন বোর্ডে বাল্ক ফিচার করুন।' 
                  : 'Bulk feature all active listings of members subscribed to a specific package with a single click.'}
              </p>
              <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                <select
                  id="bulkFeaturedPackageSelector"
                  style={{
                    flex: 1, minWidth: '280px', padding: '0.6rem 0.8rem', borderRadius: '8px',
                    border: '1px solid #cbd5e1', fontSize: '0.85rem', fontWeight: 700,
                    background: 'white', outline: 'none'
                ...
                >
                  <option value="">
                    {lang === 'bn' ? '-- মেম্বারশিপ প্যাকেজ নির্বাচন করুন --' : '-- Select Membership Package --'}
                  </option>
                  <option value="Business Member">
                    {lang === 'bn' ? 'বিজনেস মেম্বার (Business Member)' : 'Business Member'}
                  </option>
                  <option value="Gold Member">
                    {lang === 'bn' ? 'গোল্ড মেম্বার (Gold Member)' : 'Gold Member'}
                  </option>
                  <option value="Silver Member">
                    {lang === 'bn' ? 'সিলভার মেম্বার (Silver Member)' : 'Silver Member'}
                  </option>
                </select>
                <button
                  onClick={() => {
                    const selector = document.getElementById('bulkFeaturedPackageSelector');
                    if (selector && selector.value) {
                      const confirmMsg = lang === 'bn'
                        ? `আপনি কি নিশ্চিত যে '${selector.value}' মেম্বারশিপ প্যাকেজের সকল সক্রিয় বিজ্ঞাপনকে বিজ্ঞাপন বোর্ডে যুক্ত করতে চান?`
                        : `Are you sure you want to bulk feature all active listings of '${selector.value}' members?`;
                      if (window.confirm(confirmMsg)) {
                        handleBulkFeatureByPackage(selector.value);
                      }
                    } else {
                      alert(lang === 'bn' ? 'অনুগ্রহ করে একটি প্যাকেজ নির্বাচন করুন!' : 'Please select a package first!');
                    }
                  }}
                  disabled={bulkFeatureLoading}
                  style={{
                    background: '#3b82f6', color: 'white', border: 'none', borderRadius: '8px',
                    padding: '0.6rem 1.25rem', fontWeight: 800, fontSize: '0.85rem', cursor: 'pointer',
                    transition: 'all 0.2s', boxShadow: '0 2px 8px rgba(59, 130, 246, 0.2)',
                    display: 'flex', alignItems: 'center', gap: '0.4rem', opacity: bulkFeatureLoading ? 0.7 : 1
                  }}
                >
                  {bulkFeatureLoading ? (
                    ...
                    <>
                      <span>★</span>
                      <span>{lang === 'bn' ? 'বাল্ক ফিচার করুন' : 'Bulk Promote Now'}</span>
                    </>
                  )}
                </button>
              </div>
              ...
              {displayedAds.map(ad => (
                <div key={ad.id} className={styles.adCard}>
                  ...
                  <div className={styles.cardFooter} style={{ flexDirection: 'column', gap: '0.5rem' }}>
                    <div className={styles.actionBtns} style={{ width: '100%', display: 'flex', flexWrap: 'wrap', gap: '0.4rem', alignItems: 'center' }}>
                      {ad.status === 'pending' && (
                        <button 
                          ...
                          style={{ flex: '1 1 120px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '0.3rem', minHeight: '34px' }}
                        >
                      ...
                      {ad.status === 'active' && (
                        <>
                          ...
                          <div style={{ flex: '1 1 130px', minWidth: '130px' }}>
                            <select
                              value={ad.promotion_type || (ad.is_verified ? 'Premium Listing' : '')}
                              onChange={(e) => {
                                const val = e.target.value;
                                confirmAction(
                                  `প্রমোশন স্ট্যাটাস পরিবর্তন করবেন?`, 'Change Promotion Status?',
                                  `বিজ্ঞাপনটির প্রমোশন স্ট্যাটাস "${val || 'সাধারণ'}" হিসেবে আপডেট করা হবে এবং এটি সবার উপরে চলে আসবে।`,
                                  `This ad's promotion will be updated to "${val || 'Regular'}" and bumped to the top row.`,
                                  () => handleSetPromotionType(ad.id, val)
                                );
                              }}
                              disabled={actionLoading === ad.id + '-promo'}
                              style={{
                                width: '100%',
                                padding: '0.4rem 0.5rem',
                                borderRadius: '6px',
                                border: '1px solid #fde047',
                                background: ad.promotion_type || ad.is_verified ? '#fef08a' : '#f8fafc',
                                color: ad.promotion_type || ad.is_verified ? '#854d0e' : '#334155',
                                fontWeight: 800,
                                fontSize: '0.72rem',
                                cursor: 'pointer',
                                outline: 'none',
                            ...
                            >
                              <option value="">{lang === 'bn' ? '⚪ সাধারণ অ্যাড' : '⚪ Regular Ad'}</option>
                              <option value="Premium Listing">{lang === 'bn' ? '★ প্রিমিয়াম লিস্টিং' : '★ Premium Listing'}</option>
                              <option value="3-Day Express Boost">{lang === 'bn' ? '⚡ ৩ দিনের এক্সপ্রেস বুস্ট' : '⚡ 3-Day Express'}</option>
                              <option value="7-Day Premium Boost">{lang === 'bn' ? '🔥 ৭ দিনের প্রিমিয়াম বুস্ট' : '🔥 7-Day Premium'}</option>
                              <option value="15-Day Mega Boost">{lang === 'bn' ? '🚀 ১৫ দিনের মেগা বুস্ট' : '🚀 15-Day Mega'}</option>
                            </select>
                          </div>

                          {/* Featured Slider Controls Box */}
                          {(() => {
                            const featuredItem = featuredList.find(item => item.listing_id === ad.id);
                            
                            return (
                              <div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap', flex: '1 1 180px' }}>
                                <button 
                                  className={`${styles.btn}`}
                                  style={{ 
                                    flex: '1 1 100px', 
                                    background: featuredItem ? '#d97706' : '#6366f1', 
                                    color: 'white', 
                                    border: 'none',
                                    display: 'inline-flex',
                                    alignItems: 'center',
                                  ...
                                  onClick={(e) => { 
                                    e.preventDefault(); e.stopPropagation(); 
                                    handleToggleSliderDirect(ad.id);
                                  }}
                                  disabled={actionLoading === ad.id + '-slider'}
                                >
                                  <Star size={14} fill={featuredItem ? "white" : "none"} /> 
                                  {featuredItem 
                                    ? (lang === 'bn' ? 'স্লাইডারে যুক্ত' : 'In Slider') 
                                    : (lang === 'bn' ? 'স্লাইডারে দিন' : 'Add Slider')}
                                </button>

                                {featuredItem && (
                                  <>
                                    {/* Slider Active / Deactivate Toggle */}
                                    <button
                                      onClick={(e) => {
                                        e.preventDefault(); e.stopPropagation();
                                        handleToggleFeaturedActive(featuredItem.id, featuredItem.is_active);
                                      }}
                                      style={{
                                        background: featuredItem.is_active ? '#dcfce7' : '#fee2e2',
                                        color: featuredItem.is_active ? '#15803d' : '#b91c1c',
                                        border: `1px solid ${featuredItem.is_active ? '#86efac' : '#fca5a5'}`,
                                        borderRadius: '6px',
                                      ...
                                      title={lang === 'bn' ? 'স্লাইডার অ্যাক্টিভ/ডিঅ্যাক্টিভেট করুন' : 'Toggle Slider Active Status'}
                                    >
                                      {featuredItem.is_active ? (lang === 'bn' ? '🟢 সক্রিয়' : '🟢 Active') : (lang === 'bn' ? '🔴 নিষ্ক্রিয়' : '🔴 Off')}
                                    </button>

                                    {/* Slider Sort Order Dropdown */}
                                    <select
                                      value={featuredItem.sort_order || 0}
                                      onChange={(e) => {
                                        e.stopPropagation();
                                        handleUpdateFeaturedSortOrder(featuredItem.id, e.target.value);
                                      }}
                                      ...
                                      title={lang === 'bn' ? 'স্লাইডার পজিশন নির্বাচন করুন' : 'Select Slider Sort Position'}
                                    >
                                      <option value={0}>Pos #0 ({lang === 'bn' ? '১ম' : '1st'})</option>
                                      <option value={1}>Pos #1 ({lang === 'bn' ? '২য়' : '2nd'})</option>
                                      <option value={2}>Pos #2 ({lang === 'bn' ? '৩য়' : '3rd'})</option>
                                      <option value={3}>Pos #3 ({lang === 'bn' ? '৪র্থ' : '4th'})</option>
                                      <option value={4}>Pos #4 ({lang === 'bn' ? '৫ম' : '5th'})</option>
                                      <option value={5}>Pos #5 ({lang === 'bn' ? '৬ষ্ঠ' : '6th'})</option>
                                    </select>
                                  </>
                                )}
                              </div>
